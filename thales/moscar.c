...
void proceso(char *aci, struct trans *tx_in, struct trans *tx_out, struct trans *tx_sa)
{
   ...
   { 
      ...
      pid = getpid();
      save("moscar",&pid,"TXIN",tx_in->datos);
      memset(&mensaje,0,sizeof mensaje);
      memset(&respuesta,0,sizeof respuesta);
      sprintf(mensaje.texto.datos,"%6s","moscar");
      mensaje.mtype = 1; 
	  ...
	  //tx_out->len = sprintf(tx_out->datos,"%s","hola");
	  	 tx_out->len = sprintf(tx_out->datos,"%s",respuesta.texto.datos);
		 	//  tx_out->len = sprintf(tx_out->datos,"%2s%60s%1000s",respuesta.texto.datos);
			  	 // tx_out->len = sprintf(tx_out->datos,"%1062s",respuesta.texto.datos);
