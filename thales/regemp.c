...
sscanf 
(tx_in->datos,"%30c %10c %30c %15c %30c %15c %60c %60c %15c",nombre,rut,fecha_n,sueldo,cargo,telefono,direccion,correo,estado);
...
int qid,pid;
	...
	sprintf(mensaje.texto.dat, "%6s%s%s%s%s%s%s%s%s%s", "regemp", nombre, rut, fecha_n, 
