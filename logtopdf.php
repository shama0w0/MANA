...
$pdf->Cell(0,10,'LISTADO DE LOGS',0,0,'C');
$pdf->Ln(20);
...
while ($row = pg_fetch_array($result, null, PGSQL_ASSOC))
	...
	$pdf->Cell(190,10,'LOG N°: '.$auxcont,1,0,'C');
	$pdf->SetFont('Helvetica','I',13); 
	$pdf->Ln(15);
	$pdf->Cell(100,10,'Origen: '.$row["origen"]);
	$pdf->Cell(100,10,'Nombre Usuario: '.$row["nombre_user"]);
	$pdf->Ln(10);
	$pdf->Cell(100,10,'Nombre Producto: '.$row["nombre_pro"]);
	$pdf->Ln(10);
	$pdf->Cell(100,10,'Codigo: '.$row["codigo_pro"]);
	$pdf->Cell(100,10,'Fecha del Cambio: '.$row["fecha"]);
	$pdf->Ln(10);
	$pdf->MultiCell(0,10,'Razon del Cambio: '.$row["razon"]);
	$pdf->Ln(20);
	++$contador;
	++$auxcont;
	if($contador==3)
